...
{
    public class DeleteUserEmergencyContactHandlerTests
    {
        ...
        private readonly DeleteUserEmergencyContactHandler _handler;

        public DeleteUserEmergencyContactHandlerTests()
        {
            ...
            _handler = new DeleteUserEmergencyContactHandler(_writeRepositoryFactoryMock.Object, _unitOfWorkMock.Object);
        }

        [Fact]
        public async Task Handle_ShouldDeleteUserEmergencyContact_WhenValidRequestIsProvided()
        {
            // Arrange
            var command = new DeleteUserEmergencyContactCommand(id: Guid.NewGuid());

            _repoMock.Setup(r => r.ExecuteAsync(It.IsAny<string>(), It.IsAny<CancellationToken>(), It.IsAny<object>(), It.IsAny<IDbTransaction>(), "text"))
                     .ReturnsAsync(1); // Assume 1 row was affected (successful deletion)

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            result.Should().Be(1); // One row should be deleted

            // Verify that the correct SQL was executed
            _repoMock.Verify(r => r.ExecuteAsync(It.Is<string>(sql => sql.Contains("DELETE FROM [dbo].[UserEmergencyContacts]")),
        ...
        public async Task Handle_ShouldReturnZero_WhenNoRowsAffected()
        {
            ...
            _repoMock.Setup(r => r.ExecuteAsync(It.IsAny<string>(), It.IsAny<CancellationToken>(), It.IsAny<object>(), It.IsAny<IDbTransaction>(), "text"))
                     .ReturnsAsync(0); // No rows affected (no matching record found)

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            result.Should().Be(0); // No rows were deleted
        ...
        public async Task Handle_ShouldThrowException_WhenDeleteFails()
        {
            ...
            // Verify that CommitAsync was not called because the transaction should not be committed on failure
