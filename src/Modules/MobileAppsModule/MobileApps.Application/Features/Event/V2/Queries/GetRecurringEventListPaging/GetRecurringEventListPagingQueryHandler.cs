...
{
    class GetRecurringEventListPagingQueryHandler : IRequestHandler<GetRecurringEventListPagingQuery, IList<IDictionary<string, object>>>
    {
        private readonly LazyService<IReadRepository<object>> _readRepository;
        private IMediator _mediator;
        private readonly ISystemSettingsService _systemSettingsService;
        public GetRecurringEventListPagingQueryHandler(LazyService<IReadRepository<object>> readRepository, IMediator mediator
        ...
        public async Task<IList<IDictionary<string, object>>> Handle(GetRecurringEventListPagingQuery request, CancellationToken cancellationToken)
        {
        ...
        private static string GetRecurringEventSql()
        {
            ...
            (
                SELECT 
                    ...
                    CASE 
                        WHEN CAST(SYSUTCDATETIME() AS DATE) > CAST(od.ScheduleDate AS DATE) THEN -1
                        WHEN CAST(SYSUTCDATETIME() AS DATE) = CAST(od.ScheduleDate AS DATE) THEN 1
                        ELSE 0
                    END AS IsTodayEvent,

                    ROW_NUMBER() OVER (
                        PARTITION BY ed.DocId
                        ORDER BY od.ScheduleDate DESC, od.RecurringStartTime DESC
                    ) AS rn
            ...
            (
                SELECT 
                    ...
                    ROW_NUMBER() OVER (
                        ORDER BY 
                             CASE 
                                WHEN CAST(SYSUTCDATETIME() AS DATE) > CAST(StartDate AS DATE) THEN -1
                                WHEN CAST(SYSUTCDATETIME() AS DATE) = CAST(StartDate AS DATE) THEN 1
                                ELSE 0
                             END DESC,
                             StartDate DESC,
