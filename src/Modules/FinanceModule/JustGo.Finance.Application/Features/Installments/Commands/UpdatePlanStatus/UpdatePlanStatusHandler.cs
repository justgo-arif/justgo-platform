...
namespace JustGo.Finance.Application.Features.Installments.Commands.CancelledInstallment
{
    ...
    public class UpdatePlanStatusHandler : IRequestHandler<UpdatePlanStatusCommand, bool>
    {
        ...
        public UpdatePlanStatusHandler(IMediator mediator, IWriteRepositoryFactory writeRepository, IUnitOfWork unitOfWork, IUtilityService utilityService)
        {
            _mediator = mediator;
        ...
        public async Task<bool> Handle(UpdatePlanStatusCommand request, CancellationToken cancellationToken)
        {
            ...
            var status = request.IsActive ? PlanStatus.Active : PlanStatus.Inactive;
           ...
           .ExecuteAsync(sql, cancellationToken, parameters, dbTransaction, "text");
            ...
            var message = rowsAffected > 0
                        ? $"Status of Recurring Payment Plan (PlanId: {request.PlanId}) successfully updated to Active."
