...
{
    public class GetShippingAddressHandler : IRequestHandler<GetShippingAddressQuery, Address>
    {
        private readonly LazyService<IReadRepository<Address>> _readRepository;
        private readonly IMediator _mediator;

        public GetShippingAddressHandler(LazyService<IReadRepository<Address>> readRepository, IMediator mediator)
        {
            _readRepository = readRepository;
            _mediator = mediator;
        }

        public async Task<Address> Handle(GetShippingAddressQuery request, CancellationToken cancellationToken)
        {
            ...
            if (address is null)
                throw new InvalidOperationException("Shipping address not found for the provided Document Id.");
            return address;
        }
    }
}
