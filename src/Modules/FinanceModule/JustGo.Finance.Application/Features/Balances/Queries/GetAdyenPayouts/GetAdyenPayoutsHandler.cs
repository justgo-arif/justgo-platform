...
{
    public class GetAdyenPayoutsHandler : IRequestHandler<GetAdyenPayoutsQuery, AdyenPayoutsDTO>
    {
        private readonly LazyService<IReadRepository<AdyenPayoutDetails>> _readRepository;
        private readonly IMediator _mediator;
        public GetAdyenPayoutsHandler(
            LazyService<IReadRepository<AdyenPayoutDetails>> readRepository, 
            IMediator mediator
            )
        {
        ...
        public async Task<AdyenPayoutsDTO> Handle(GetAdyenPayoutsQuery request, CancellationToken cancellationToken)
        {
            var adyenPayouts = new AdyenPayoutsDTO();
            ...
            adyenPayouts.TotalCount = Convert.ToInt32(countResult);
            ...
            var adyenPayoutsRes = await _readRepository.Value.GetListAsync(
                dataQuery, cancellationToken, queryParameters, null, "text");

            adyenPayouts.Payouts = adyenPayoutsRes.ToList();
            adyenPayouts.PageNo = request.PageNo;
            adyenPayouts.PageSize = request.PageSize;
