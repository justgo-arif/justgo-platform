...
namespace JustGo.Result.Application.Features.Events.Commands.AddEventCompetition;

public class AddEventCompetitionCommandHandler : IRequestHandler<AddEventCompetitionCommand, Result<AddEventCompetitionResponse>>
{
    ...
    private readonly ILogger<AddEventCompetitionCommandHandler> _logger;
    ...
    private const string InsertMatchSql = """
        INSERT INTO ResultCompetitionMatches (RoundId, CompetitionParticipantId, CompetitionParticipantId2, WinnerCompetitionParticipantId)
        VALUES (@RoundId, @Player1ParticipantId, @Player2ParticipantId, @WinnerParticipantId);
        SELECT CAST(SCOPE_IDENTITY() AS INT);
        """;
    ...
    private const string InsertMatchRatingSql = """
        ...
        INSERT INTO ResultCompetitionMatchRatings (MatchId, CompetitionId, CompetitionParticipantId, UserId, RatingChange, IsWinner)
        VALUES (@MatchId, @CompetitionId, @ParticipantId, @ResolvedUserId, @RatingChange, @IsWinner);
        """;
    ...
    public async Task<Result<AddEventCompetitionResponse>> Handle(AddEventCompetitionCommand request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var matchId = await InsertMatchAsync(request.RoundId, player1ParticipantId, player2ParticipantId,
                winnerParticipantId, transaction, cancellationToken);

            if (!string.IsNullOrEmpty(request.MatchScores))
            {
                await InsertMatchMetaDataAsync(matchId, request.MatchScores, transaction, cancellationToken);
            }

            await InsertMatchRatingsAsync(
                matchId,
            ...
            var matchDetails = await GetMatchDetailsAsync(matchId, cancellationToken);

            var response = new AddEventCompetitionResponse
            {
                MatchId = matchId,
                IsSuccess = true,
                Message = "Event competition match added successfully.",
            ...
            CustomLog.Event(
                ...
                $"Event competition match created successfully for EventId: {request.EventId}, CompetitionId: {request.CompetitionId}, MatchId: {matchId}"
            );
        ...
        catch (Exception ex)
        {
            ...
            CustomLog.Event(
                ...
                $"Exception occurred while adding event competition match for EventId: {request.EventId}, CompetitionId: {request.CompetitionId}. Error: {ex.Message}"
            );
            return Result<AddEventCompetitionResponse>.Failure("Failed to add event competition match.", ErrorType.BadRequest);
        }
    }

    private async Task<Result<AddEventCompetitionResponse>> ValidateInputsAsync(
        AddEventCompetitionCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (eventExists == 0)
        {
            return Result<AddEventCompetitionResponse>.Failure("Event not found.", ErrorType.NotFound);
        }
        ...
        {
            return Result<AddEventCompetitionResponse>.Failure("Competition not found for the specified event.", ErrorType.NotFound);
        }
        ...
        if (roundExists == 0)
        {
            return Result<AddEventCompetitionResponse>.Failure("Round not found.", ErrorType.NotFound);
        }
        ...
        if (player1Exists == 0)
        {
            return Result<AddEventCompetitionResponse>.Failure("Player 1 not found.", ErrorType.NotFound);
        }
        ...
        if (player2Exists == 0)
        {
            return Result<AddEventCompetitionResponse>.Failure("Player 2 not found.", ErrorType.NotFound);
        }

        if (request.WinnerUserId != request.Player1UserId &&
            request.WinnerUserId != request.Player2UserId)
        {
            return Result<AddEventCompetitionResponse>.Failure("Winner must be one of the two players.", ErrorType.BadRequest);
        }


        return new AddEventCompetitionResponse();
    }
    ...
    private async Task<int> InsertMatchAsync(int roundId, int player1ParticipantId, int player2ParticipantId, int winnerParticipantId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var matchId = await writeRepo.ExecuteScalarAsync<int>(InsertMatchSql, cancellationToken, parameters, transaction, QueryType.Text);

        return matchId;
    }

    private async Task InsertMatchMetaDataAsync(int matchId, string matchScores,
        System.Data.Common.DbTransaction transaction,
        CancellationToken cancellationToken)
    {
        var writeRepo = _writeRepositoryFactory.GetRepository<object>();
        ...
        parameters.Add("@Key", MatchScoresKey);
        parameters.Add("@Value", matchScores);
        parameters.Add("@DataType", StringValue);

        await writeRepo.ExecuteAsync(InsertMatchMetaDataSql, cancellationToken, parameters, transaction, QueryType.Text);
    }

    private async Task InsertMatchRatingsAsync(
        int matchId,
        int competitionId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var userId in new[] { player1UserId, player2UserId  })
        {
            var parameters = new DynamicParameters();
    ...
    private async Task<CompetitionMatchDto?> GetMatchDetailsAsync(int matchId, CancellationToken cancellationToken)
    {
        ...
        var sql = $"""
            SELECT 
                rcm.MatchId,
                rcrp1.EntityId AS WinnerParticipantId,
                ...
                rcrp2.EntityId AS LoserParticipantId,
