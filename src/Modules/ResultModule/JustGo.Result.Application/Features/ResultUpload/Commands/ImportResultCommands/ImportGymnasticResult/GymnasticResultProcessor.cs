...
namespace JustGo.Result.Application.Features.ResultUpload.Commands.ImportResultCommands.ImportGymnasticResult;

public class GymnasticResultProcessor : IResultProcessor
{
    ...
    public GymnasticResultProcessor(IReadRepositoryFactory readRepoFactory,
    ...
    public async Task<Result<string>> ProcessAsync(ImportResultFileCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var uploadedFile = await ImportResultHelper.GetResultUploadedFileByFileId(request.FileDto.FileId,
                _readRepoFactory,
                cancellationToken);
        ...
        string fileDtoWebSocketId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var memberIdHeader = await ImportResultHelper.GetMemberIdColumnNameByScopeId(readRepoFactory,
                uploadedFile.DisciplineId,
                cancellationToken);
            ...
            {
                validator = new FileDataValidator(config);
            }
