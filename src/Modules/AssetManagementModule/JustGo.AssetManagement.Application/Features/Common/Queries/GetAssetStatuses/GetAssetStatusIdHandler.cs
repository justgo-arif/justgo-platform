...
{
    public class GetAssetStatusIdHandler : IRequestHandler<GetAssetStatusIdQuery, int>
    {
        private readonly LazyService<IReadRepository<AssetStatus>> _readRepository;

        public GetAssetStatusIdHandler(LazyService<IReadRepository<AssetStatus>> readRepository)
        {
            _readRepository = readRepository;
        }

        public async Task<int> Handle(GetAssetStatusIdQuery request, CancellationToken cancellationToken)
        {
            string sql = @"SELECT * FROM [dbo].[AssetStatus] WHERE Type = 1 and Name = @Name";
