...
{
    public class GetOwnerTransferApprovalMetadataQueryHandler : IRequestHandler<GetOwnerTransferApprovalMetadataQuery, List<OwnerTransferApprovalMetadataDTO>>
    {
        private readonly IReadRepositoryFactory _readRepository;
        private IUtilityService _utilityService;

        public GetOwnerTransferApprovalMetadataQueryHandler(
        ...
        public async Task<List<OwnerTransferApprovalMetadataDTO>> Handle(GetOwnerTransferApprovalMetadataQuery request, CancellationToken cancellationToken)
        {
            int currentUserId = await _utilityService.GetCurrentUserId(cancellationToken);

            string sql = @" declare @assetTransferId int = (select AssetOwnershipTransferId from AssetOwnershipTransfers 
                            ...
                            ws.StepId,
                            u.UserId UserDocId,
                            CAST(u.UserSyncId AS nvarchar(200)) UserId,
                            ...
                            we.ActionStatus,
                            we.Remarks,
							max(case when fm.UserId is not null then 1 else 0 end) IsFamilyMember,
							max(case when uc.ClubId is not null then 1 else 0 end) IsAdmin,
							max(case when u.Userid = @CurrentUserId then 1 else 0 end) IsCurrentUser
                            ...
                            where ws.WorkFlowType = 8 and ws.ResourceId = @assetTransferId
                            group by 
                            ws.StepId,
                            u.UserId,
                            u.UserSyncId,
            ...
            queryParameters.Add("@recordGuid", request.TransferId);
            queryParameters.Add("@currentUserId", currentUserId);
            var result = (await _readRepository
                                .GetLazyRepository<OwnerTransferApprovalMetadataDTO>()
