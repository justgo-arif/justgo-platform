...
public class GetMfaByUserIdQueryHandler : IRequestHandler<GetMfaByUserIdQuery, UserMFA>
{
    private readonly LazyService<IReadRepository<UserMFA>> _readRepository;
    private readonly IUtilityService _utilityService;
    public GetMfaByUserIdQueryHandler(LazyService<IReadRepository<UserMFA>> readRepository, IUtilityService utilityService)
    {
        _readRepository = readRepository;
        _utilityService = utilityService;
    }

    public async Task<UserMFA> Handle(GetMfaByUserIdQuery request, CancellationToken cancellationToken)
    {
        string sql = @"DECLARE @NotAllowedRoles NVARCHAR(MAX) = (SELECT Value FROM SystemSettings WHERE [ItemKey] = 'SYSTEM.MFA.NOTALLOWEDROLE');
                        ...
                        BackUpCode,whatsAppState,AuthenticatorAppState,WhatsAppNumber,PhoneCode,CountryCode, Email, IsEmailAuthEnabled, EmailAuthState, 
                        @IsVisibleEmailMFA AS 'IsVisibleEmailMFA', @IsVisibleAuthenticatorMFA AS 'IsVisibleAuthenticatorMFA', @IsVisibleWhatsappMFA AS 'IsVisibleWhatsappMFA'  
                        from [userMFA] 
                        mfa inner join [User] u on u.Userid=mfa.userid
                        OUTER APPLY
                        (select top  1 gm_offset,abbreviation from Timezone where mfa.EnableWhatsappDate IS NOT NULL and time_start <= cast(DATEDIFF(HOUR,'1970-01-01 00:00:00', mfa.EnableWhatsappDate) as bigint)*60*60
                        and zone_id=@TimeZone order by time_start desc) as X
                        OUTER APPLY
                        (select top  1 gm_offset,abbreviation from Timezone where mfa.EnableAuthenticatorAppDate IS NOT NULL and time_start <=  cast(DATEDIFF(HOUR,'1970-01-01 00:00:00', mfa.EnableAuthenticatorAppDate) as bigint)*60*60
                        and zone_id=@TimeZone order by time_start desc) as Y
                        OUTER APPLY
                        (select top  1 gm_offset,abbreviation from Timezone where mfa.EmailAuthEnableDate IS NOT NULL and time_start <=  cast(DATEDIFF(HOUR,'1970-01-01 00:00:00', mfa.EmailAuthEnableDate) as bigint)*60*60
