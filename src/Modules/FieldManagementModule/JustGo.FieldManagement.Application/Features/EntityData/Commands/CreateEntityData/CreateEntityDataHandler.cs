...
{
    public class CreateEntityDataHandler : IRequestHandler<CreateEntityDataCommand, int>
    {
        ...
        public CreateEntityDataHandler(
            IMediator mediator,
        ...
        public async Task<int> Handle(CreateEntityDataCommand request, CancellationToken cancellationToken)
        {
            var result = await SaveExtensionData(request.ExId, request.DocId, request.Data, cancellationToken);
        ...
        private async Task<int> SaveExtensionData(int exId, int docId, Dictionary<string, object> Data, CancellationToken cancellationToken)
        {
            using var transaction = await _unitOfWork.BeginTransactionAsync();
            var schema = await _mediator.Send(new GetEntityExtensionSchemaByIdQuery(exId, false), cancellationToken);
            if (schema is null)
            {
                _error.NotFound<object>("Schema not found.");
                return 0;
            ...
            foreach (var dt in Data)
            {
                ...
                if (dt.Key.StartsWith("$"))
                {
                    ...
                    await SaveFieldSetDataAsync(exId, docId, dt, schema, cancellationToken, transaction);
                    continue;
                }
        ...
        private async Task SaveFieldSetDataAsync(int exId, int docId, KeyValuePair<string, object> fieldSetData, EntityExtensionSchema schema, CancellationToken cancellationToken, IDbTransaction transaction)
        {
            ...
            foreach (var row in rowData)
            {
                ...
                foreach (var fv in row.Value)
                {
                    ...
                    else
                    {
                        ...
                        currencyParams.Add("DocId", docId);
